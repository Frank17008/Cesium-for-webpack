...
Cesium.Ion.defaultAccessToken =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiIyNjM2OGFlOC1lZDY4LTQ2NjQtODQ2Yi1iYTM0YTQzNGE3OWEiLCJpZCI6MjYyMzAsInNjb3BlcyI6WyJhc3IiLCJnYyJdLCJpYXQiOjE1ODc0NTQ3NDB9.9KgD-eLN196Tz-eDSA_NLt33E2QyditDcTTEjh4L_TA";
const EVENTS_MAP = {
    click: "LEFT_CLICK",
...
class Map3D {
    ...
    showNavigation() {
        ...
        ).parentElement.title = "放大";
        document.querySelector(
            ".navigation-control-icon-zoom-out"
        ).parentElement.title = "缩小";
    ...
    showMouseLocation() {
        ...
        this.registerEvent("mousemove", (movement) => {
            ...
            let cartesian = self.viewer.camera.pickEllipsoid(
                movement.endPosition,
                ellipsoid
            );
            if (cartesian) {
                //将笛卡尔三维坐标转为地图坐标（弧度）
                let cartographic = self.viewer.scene.globe.ellipsoid.cartesianToCartographic(
                    cartesian
                );
                //将地图坐标（弧度）转为十进制的度数
                let lat_String = Cesium.Math.toDegrees(
                    cartographic.latitude
                ).toFixed(4);
                let log_String = Cesium.Math.toDegrees(
                    cartographic.longitude
                ).toFixed(4);
                let alti_String = (
                    self.viewer.camera.positionCartographic.height / 1000
                ).toFixed(2);
    ...
    addMarkers(markers, dataSource = null) {
        if (markers.length) {
            ...
            markers.forEach((marker, index) => {
                ...
                entities[index] = dataSource
                    ? dataSource.entities.add(entryObj)
                    : this.viewer.entities.add(entryObj);
            });
            return entities;
        }
    }
    ...
    setColor(markerIds, color) {
        markerIds &&
            ...
            markerIds.forEach((markerId) => {
                ...
                if (marker) {
                    marker._billboard.color = Cesium.Color.fromCssColorString(
                        color
                    );
                }
            });
    ...
    registerEvent(type, func) {
        if (type && EVENTS_MAP[type]) {
            ...
            if (!this.eventHandler[type]) {
                this.eventHandler[type] = new Cesium.ScreenSpaceEventHandler(
                    canvas
                );
    ...
    addInfowindow(entity, position, config) {
        ...
        if (!infoDiv) {
            ...
            infoDiv.className = config.className
                ? `${config.className} infowindow`
                : "infowindow";
